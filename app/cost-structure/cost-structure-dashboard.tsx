...
type Dashboard = {
  ...
  upload: { version: number; fileName: string };
...
export default function CostStructureDashboard({ periods }: { periods: PeriodOption[] }) {
  ...
  const runAction = async (action: 'reconcile' | 'finalize' | 'reopen') => {
    ...
    try {
      const response = await fetch(`/api/cost-structure/periods/${data.period.id}/${action}`, { method: 'POST', headers: body ? { 'content-type': 'application/json' } : undefined, body });
