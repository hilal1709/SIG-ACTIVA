...
export default function FluctuationWorkflow({ periodOptions }: { periodOptions: PeriodOption[] }) {
  ...
  function resetAnalysis() { setData(null); setSelected(''); setReason(''); setNote(''); }
  function changeCompany(nextCompany: string) { setCompanyCode(nextCompany); const latestPeriod = periodOptions.find((period) => period.companyCode === nextCompany); setPeriodId(latestPeriod ? String(latestPeriod.id) : ''); resetAnalysis(); }
  async function load() { setLoading(true); const response = await fetch(`/api/cost-fluctuation/commentary?periodId=${periodId}&comparison=${comparison}`); setData(await response.json()); setLoading(false); }
  async function mutate(path: string, body: object = {}) { setLoading(true); const response = await fetch(path, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) }); if (!response.ok) { const result = await response.json(); setData((old) => ({ ...old, error: result.error })); } else await load(); setLoading(false); }
  ...
  return <section className="space-y-4 rounded-xl border bg-white p-5">
    <div className="flex flex-wrap gap-3">
      <select className="min-w-48 rounded border px-3 py-2" value={companyCode} onChange={(event) => changeCompany(event.target.value)}><option value="">Pilih Company Code</option>{companies.map((code) => <option key={code} value={code}>{code}</option>)}</select>
      <select className="min-w-52 rounded border px-3 py-2" value={periodId} disabled={!companyCode} onChange={(event) => { setPeriodId(event.target.value); resetAnalysis(); }}><option value="">Pilih Periode</option>{companyPeriods.map((period) => <option key={period.id} value={period.id}>{period.fiscalYear}/{String(period.fiscalPeriod).padStart(2, '0')}</option>)}</select>
    ...
    {data?.hierarchy && <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-2">Analytical target</th><th>Variance</th><th>Variance %</th><th>Materiality</th><th>Commentary</th></tr></thead><tbody>{flatten(data.hierarchy).map((row) => { const selectable = isCommentaryTarget(row); return <tr className={`${selectable ? 'cursor-pointer hover:bg-slate-50' : 'cursor-default bg-slate-50/50'} border-b`} key={row.key} onClick={() => { if (!selectable) return; setSelected(row.key); setReason(data.commentaries?.find((item) => item.analysisKey === row.key)?.reason ?? ''); }}><td className="p-2" style={{ paddingLeft: 8 + row.depth * 18 }}>{row.label}</td><td>{row.varianceAmount}</td><td>{row.variancePercent ?? 'N/M'}</td><td>{row.materialityStatus}</td><td>{selectable ? statuses.get(row.key) ?? (row.materialityStatus === 'REQUIRES_EXPLANATION' ? 'OPEN' : '—') : '—'}</td></tr>; })}</tbody></table></div>}
