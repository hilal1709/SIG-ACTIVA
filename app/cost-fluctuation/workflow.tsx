...
export default function FluctuationWorkflow() {
  ...
  return <section className="space-y-4 rounded-xl border bg-white p-5">
    <div className="flex flex-wrap gap-3">
      <input className="rounded border px-3 py-2" inputMode="numeric" placeholder="Period ID" value={periodId} onChange={(event) => setPeriodId(event.target.value)} />
      <select className="rounded border px-3 py-2" value={comparison} onChange={(event) => setComparison(event.target.value)}><option>MOM</option><option>YOY</option><option>YTD</option></select>
      <button className="rounded bg-blue-700 px-4 py-2 text-white disabled:opacity-50" onClick={load} disabled={loading || !periodId}>{loading ? 'Memuat…' : 'Tampilkan analisis'}</button>
    </div>
    ...
    <button className="rounded border px-3 py-2" onClick={() => mutate('/api/cost-fluctuation/review/complete', { periodId: Number(periodId), note })}>Complete period review</button>
