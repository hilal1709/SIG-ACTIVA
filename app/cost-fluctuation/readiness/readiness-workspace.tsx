...
function ReadinessCard({ title, current, value }: { title: string; current: CurrentPeriodReadiness; value: ComparisonReadiness }) {
  ...
  return <Card className="h-full">
    ...
    <CardContent className="space-y-3 text-sm">
      ...
      {current.currentReadiness !== 'AVAILABLE' && <p className="border-t pt-3"><strong>Current period:</strong> belum authoritative untuk analisis.</p>}
...
export default function ReadinessWorkspace({ data }: { data: ReadinessMatrix }) {
  ...
  const filtered = useMemo(() => data.periods.filter((item) => company === 'ALL' || item.companyCode === company), [company, data.periods]);
  const periodKeys = useMemo(() => [...new Set(filtered.map((item) => `${item.fiscalYear}-${item.fiscalPeriod}`))], [filtered]);
  const [periodKey, setPeriodKey] = useState(() => data.periods[0] ? `${data.periods[0].fiscalYear}-${data.periods[0].fiscalPeriod}` : '');
  const effectivePeriodKey = periodKeys.includes(periodKey) ? periodKey : (periodKeys[0] ?? '');
  const selected = filtered.filter((item) => `${item.fiscalYear}-${item.fiscalPeriod}` === effectivePeriodKey);

  const changeCompany = (nextCompany: string) => {
    setCompany(nextCompany);
    const first = data.periods.find((item) => nextCompany === 'ALL' || item.companyCode === nextCompany);
    setPeriodKey(first ? `${first.fiscalYear}-${first.fiscalPeriod}` : '');
  };

  return <div className="space-y-6">
    <Card><CardContent className="flex flex-wrap gap-3 pt-6">
      <label className="grid gap-1 text-sm"><span className="font-medium">Company</span><select className="min-w-40 rounded-md border bg-background px-3 py-2" value={company} onChange={(event) => changeCompany(event.target.value)}><option value="ALL">All</option>{data.companies.map((code) => <option key={code}>{code}</option>)}</select></label>
      <label className="grid gap-1 text-sm"><span className="font-medium">Current period</span><select className="min-w-48 rounded-md border bg-background px-3 py-2" value={effectivePeriodKey} onChange={(event) => setPeriodKey(event.target.value)} disabled={periodKeys.length === 0}>{periodKeys.map((key) => { const [year, period] = key.split('-').map(Number); return <option key={key} value={key}>{month(year, period)}</option>; })}</select></label>
    </CardContent></Card>
    ...
    <Card className="border-amber-200 bg-amber-50/50"><CardContent className="flex flex-col gap-3 pt-6 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-700" /><div><p className="font-medium">Lengkapi histori tanpa membuat data semu</p><p className="text-sm text-muted-foreground">Upload dan finalisasi periode Cost Structure yang hilang sebelum comparison dapat dianalisis.</p></div></div><Link href="/cost-structure/upload" className="inline-flex items-center gap-2 rounded-md border bg-background px-3 py-2 text-sm font-medium"><Upload className="h-4 w-4" />Buka Upload Data</Link></CardContent></Card>
