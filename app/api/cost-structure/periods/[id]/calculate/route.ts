...
import { CalculationConflictError, runCompany2000Calculation } from '@/lib/cost-structure/calculations/run-service';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const calculation = await runCompany2000Calculation(id, auth.user.uid);
