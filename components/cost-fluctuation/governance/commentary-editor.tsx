...
export type CommentaryView = { id: number; analysisKey: string; status: string; reason: string; reviewerNote?: string | null; preparedBy?: { name: string }; reviewedBy?: { name: string }; history?: Array<{ id: number; version: number; status: string; reason: string; reviewerNote?: string | null; createdAt?: string }> };

export function CommentaryEditor({ analysisKey, commentary, permissions, busy, onAction }: { analysisKey: string; commentary?: CommentaryView; permissions: GovernancePermissions; busy: boolean; onAction: (action: 'draft'|'submit'|'return'|'review', payload?: string) => Promise<void> }) {
  const [reason, setReason] = useState(commentary?.reason ?? '');
  const [note, setNote] = useState('');
  const actions = commentaryActions(commentary?.status, permissions);
