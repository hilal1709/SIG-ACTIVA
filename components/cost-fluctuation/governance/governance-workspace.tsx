  ...
  return <section className="space-y-5">
    ...
    {data?.status === 'AVAILABLE' && <>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_380px]">
        ...
        {selected ? <CommentaryEditor key={`${selected}:${current?.id ?? 'new'}:${current?.status ?? 'OPEN'}`} analysisKey={selected} commentary={current} suggestedText={flatten(data.hierarchy ?? []).find(row=>row.key===selected)?.suggestedCommentary?.text} permissions={permissions} currentUserId={currentUserId} busy={loading} onAction={action} /> : <div className="rounded-xl border border-dashed p-5 text-sm text-slate-500">Pilih Cost Group, Nature, COA, atau Calculated Item. GHOPO dan DERIV tetap terpisah melalui basis-qualified analysis key.</div>}
      </div>
    </>}
