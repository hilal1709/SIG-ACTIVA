...
export interface Lineage extends MonthRef { periodId: number; runId: number; ruleSetVersion: string }
...
export interface AnalyticalSnapshot { companyId: number; companyCode: string; amount: Prisma.Decimal; groups: SnapshotGroup[]; lineage: Lineage[] }
...
export interface ComparedNode extends ComparedMetric { key: string; id: number | null; code: string; label: string; nodeType: 'COMPANY' | 'COST_GROUP' | 'NATURE' | 'COA' | 'CALCULATED_ITEM'; order: number; children?: ComparedNode[]; lineType?: string; ruleCode?: string | null }
...
export interface PersistedPeriod extends MonthRef { id: number; companyId: number; companyCode: string; status: string; activeCalculationRunId: number | null; activeRun: null | { id: number; periodId: number; status: string; isActive: boolean; ruleSetVersion: string; results: PersistedResult[]; actualLines: PersistedLine[] } }
