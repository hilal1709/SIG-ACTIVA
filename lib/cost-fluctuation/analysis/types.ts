...
export interface PersistedPeriod extends MonthRef { id: number; companyId: number; companyCode: string; status: string; activeCalculationRunId: number | null; activeRun: null | { id: number; periodId: number; uploadId: number; status: string; isActive: boolean; ruleSetVersion: string; results: PersistedResult[]; actualLines: PersistedLine[]; sourceRows: PersistedSourceRow[] } }
