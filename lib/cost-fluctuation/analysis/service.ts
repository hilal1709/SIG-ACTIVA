...
import { loadFinalizedMonthlySnapshot } from './snapshot';
...
export async function getCostFluctuationAnalysis(periodId: number, comparisonType: ComparisonType) {
  const requested = await prisma.costPeriod.findUnique({ where: { id: periodId }, select: { id: true, companyId: true, fiscalYear: true, fiscalPeriod: true, status: true } });
  ...
  const load = async (months: MonthRef[]) => Promise.all(months.map(async (month) => ({ month, snapshot: await loadFinalizedMonthlySnapshot(requested.companyId, month) })));
