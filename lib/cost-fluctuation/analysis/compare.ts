...
const tree = (snapshot: AnalyticalSnapshot): TreeItem => ({ key: `company:${snapshot.companyId}`, id: snapshot.companyId, code: snapshot.companyCode, label: `Company ${snapshot.companyCode}`, amount: snapshot.amount, nodeType: 'COMPANY', children: snapshot.groups.map((g) => ({ ...g, nodeType: 'COST_GROUP', children: g.natures.map((n) => ({ ...n, nodeType: 'NATURE', children: n.items.map((i) => ({ ...i, nodeType: i.id ? 'COA' : 'CALCULATED_ITEM' })) })) })) });
...
function compareNode(current: TreeItem | undefined, comparison: TreeItem | undefined, parentVariance?: import('@prisma/client').Prisma.Decimal): ComparedNode {
  ...
  const keys = [...new Set([...currentChildren.keys(), ...comparisonChildren.keys()])].sort();
  return { key: identity.key, id: identity.id, code: identity.code, label: identity.label, nodeType: identity.nodeType, lineType: identity.lineType, ruleCode: identity.ruleCode,
