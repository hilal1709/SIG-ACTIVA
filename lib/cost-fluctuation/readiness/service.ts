...
export function createReadinessService(repository: ReadinessRepository) {
  return async function getReadiness(): Promise<ReadinessMatrix> {
    ...
    const summarize = (required: PeriodCheck[], currentReadiness: ReadinessState): ComparisonReadiness => {
      const group = (state: ReadinessState) => required.filter((item) => item.readiness === state);
      const dependencyReadiness = priority.find((state) => group(state).length > 0) ?? 'AVAILABLE';
      // Engine 2 validates the selected current period before resolving comparison history.
      // Therefore a non-authoritative current period must never be presented as comparison-ready,
      // even when all historical dependencies happen to be available.
      const readiness = currentReadiness === 'AVAILABLE' ? dependencyReadiness : currentReadiness;
      return {
        readiness, required,
    ...
    const result: CurrentPeriodReadiness[] = periods.map((period) => {
      ...
      const currentReadiness = classifyPeriod(period).readiness;
      ...
      return {
        ...
        currentReadiness,
        mom: summarize(refs(dependencies.mom), currentReadiness),
        yoy: summarize(refs(dependencies.yoy), currentReadiness),
        ytd: summarize(refs(ytdRequired), currentReadiness),
      };
