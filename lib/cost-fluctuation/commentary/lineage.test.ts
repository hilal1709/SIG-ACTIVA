...
type Row = { id: number; companyCode: string; fiscalYear: number; fiscalPeriod: number; status: string; activeCalculationRunId: number; runId: number; uploadId: number; runStatus: string; isActive: boolean; ruleSetVersion: string; hasDeriv: boolean };
const row2000: Row = { id: 1, companyCode: '2000', fiscalYear: 2026, fiscalPeriod: 7, status: 'FINALIZED', activeCalculationRunId: 10, runId: 10, uploadId: 1, runStatus: 'SUCCESS', isActive: true, ruleSetVersion: 'ENGINE1_2000_V2', hasDeriv: false };
const row7000: Row = { id: 2, companyCode: '7000', fiscalYear: 2026, fiscalPeriod: 7, status: 'FINALIZED', activeCalculationRunId: 8, runId: 8, uploadId: 2, runStatus: 'SUCCESS', isActive: true, ruleSetVersion: 'ENGINE1_7000_V1', hasDeriv: true };
const row7000Historical: Row = { ...row7000, hasDeriv: false };
...
test('commit-time lineage accepts SI, modern GHOPO+DERIV, and historical GHOPO-only source sets', async () => {
  ...
  await assertCurrentLineage(tx([row7000Historical]) as never, 'MOM', [ghopo], [], lineageKey('MOM', [ghopo], []));
});

test('missing, fabricated, duplicate, or wrong Company 7000 basis lineage is stale', async () => {
  await assert.rejects(assertCurrentLineage(tx([row7000]) as never, 'MOM', [ghopo], [], lineageKey('MOM', [ghopo], [])), /stale/);
  await assert.rejects(assertCurrentLineage(tx([row7000Historical]) as never, 'MOM', [ghopo, deriv], [], lineageKey('MOM', [ghopo, deriv], [])), /stale/);
