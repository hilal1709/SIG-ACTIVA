...
export function evaluateNode(node: ComparedNode, rule: MaterialityRuleValue | null): Pick<MaterialityNode, 'materialityStatus' | 'ruleId' | 'ruleScope' | 'criteria' | 'thresholds'> {
  if (node.nodeType === 'COMPANY') return { materialityStatus: 'NOT_APPLICABLE', ruleId: null, ruleScope: null, criteria: { amount: 'NOT_CONFIGURED', percent: 'NOT_CONFIGURED' }, thresholds: { amount: null, percent: null, operator: null } };
